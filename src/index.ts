...
if (process.env.NODE_ENV !== "test") {
  ...
  if (unsafeSecrets.includes(jwtSecret.toLowerCase())) {
    console.error(
      `❌ FATAL: Unsafe JWT_SECRET detected (known weak secret: ${jwtSecret.substring(0, 4)}...)`,
    );
    console.error(
      "   This is a known default/weak secret that must not be used.",
    );
    console.error("   EASIEST FIX: Run the secret generator:");
    console.error("   npm run generate-secrets");
    console.error("");
  ...
  if (jwtSecret.length < 32) {
    console.error(
      `❌ FATAL: JWT_SECRET is too short (${jwtSecret.length} characters).`,
    );
    console.error(
      "   Minimum 32 characters required for secure token signing.",
    );
  ...
  console.log(
    `✅ JWT_SECRET validated: ${jwtSecret.length} characters (${jwtSecret.substring(0, 4)}...)`,
  );
}
...
app.get("/", (req: Request, res: Response) => {
  ...
  res.json({
    ...
    message:
      "Welcome to WorkStation! This is an extension-first automation platform.",
    ...
    quickStart: {
      ...
      step3:
        "Use the extension popup to access Execute, Builder, Templates, History, and Settings",
    },
    webInterfaces: {
      dashboard: "/dashboard - Production React dashboard UI",
      workflowBuilder:
        "/workflow-builder.html - Visual workflow builder (also accessible via extension)",
    ...
    documentation: {
      ...
      chromeExtension:
        "See 🚀_START_HERE_CHROME_EXTENSION.md for extension details",
