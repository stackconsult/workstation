...
export function validateEnvironment(): EnvironmentConfig {
  ...
  if (!sessionSecret) {
    if (nodeEnv === 'production') {
      ...
      warnings.push('SESSION_SECRET not set - using default (only acceptable in development/test)');
    }
  } else if (sessionSecret === jwtSecret && nodeEnv === 'production') {
    errors.push('SESSION_SECRET must be different from JWT_SECRET in production (security requirement)');
  } else if (sessionSecret && sessionSecret.length < 32 && nodeEnv === 'production') {
    warnings.push(`SESSION_SECRET should be at least 32 characters (current: ${sessionSecret.length})`);
  }
  ...
  if (isNaN(dbPort) || dbPort < 1 || dbPort > 65535) {
    errors.push(`Invalid DB_PORT value: ${dbPortStr}. Must be between 1 and 65535`);
  }
  ...
  const corsOrigins = process.env.ALLOWED_ORIGINS 
    ...
    : nodeEnv === 'production' 
      ...
      : ['http://localhost:3000', 'http://localhost:3001', 'http://localhost:7042'];

  if (corsOrigins.length === 0 && nodeEnv === 'production') {
    errors.push('ALLOWED_ORIGINS must be explicitly set in production for CORS security');
  }
  ...
  return {
    ...
    sessionSecret: sessionSecret || jwtSecret || 'default-session-secret-change-this-in-production',
    jwtExpiration,
    port,
    nodeEnv,
    dbConfig: {
...
export function printEnvironmentSummary(config: EnvironmentConfig): void {
  ...
  if (config.nodeEnv !== 'production' && config.nodeEnv !== 'test') {
    ...
    console.log(`  Session Secret Length: ${config.sessionSecret.length} characters`);
    console.log(`  Database: ${config.dbConfig.user}@${config.dbConfig.host}:${config.dbConfig.port}/${config.dbConfig.name}`);
