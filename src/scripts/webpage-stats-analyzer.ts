...
async function extractGhlocStats(page: Page): Promise<GhlocStats> {
  ...
  try {
    ...
    const lineMatches = allText.match(/(\d+(?:,\d+)*)\s*lines?(?:\s+of\s+code)?/i);
    ...
    const langMatches = allText.matchAll(/(\w+):\s*(\d+(?:,\d+)*)\s*lines?\s*\((\d+\.?\d*)%\)/gi);
    stats.languages = {};
...
function compareStats(
  ...
  // Compare file counts
  if (ghlocStats.files && ghlocStats.files !== actualStats.tsFiles + actualStats.jsFiles) {
    differences.push(
...
function generateReport(report: ComparisonReport): string {
  ...
  reportLines.push(`- **${report.actualStats.totalFiles.toLocaleString()}+ tracked files** across the entire repository`);
  reportLines.push(`- **${report.actualStats.tsFiles}+ TypeScript source files** in src/`);
  reportLines.push(`- **${report.actualStats.jsFiles} JavaScript files** (build/config scripts)`);
  reportLines.push(`- **${report.actualStats.testFiles}+ test files** (.test.ts, .spec.ts)`);
  reportLines.push(`- **${report.actualStats.markdownFiles}+ documentation files** (.md)`);
  reportLines.push(`- **${report.actualStats.totalLines.toLocaleString()}+ lines of TypeScript code** in src/`);
  reportLines.push('```');
  reportLines.push('');
...
export { main, extractGitHubStats, extractGhlocStats, countActualStats, compareStats };
