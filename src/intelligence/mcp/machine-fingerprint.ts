...
export class MachineFingerprinter {
  ...
  private async captureMacOSCPU(): Promise<CPUInfo> {
    try {
      ...
      const freq = parseInt(execSync('sysctl -n hw.cpufrequency_max').toString().trim()) / 1000000;

      const capabilities: string[] = [];
      const features = execSync('sysctl -n machdep.cpu.features').toString().trim();
  ...
  private async captureLinuxCPU(): Promise<CPUInfo> {
    try {
      ...
      const arch = execSync('uname -m').toString().trim();
      const freq = parseFloat(cpuinfo.match(/CPU max MHz:\s+(\d+\.?\d*)/)?.[1] || '2400');
