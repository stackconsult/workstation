...
function registerEmailBasicsLesson(): void {
  const lesson: Lesson = {
    ...
    description: 'Learn how to automate email communication using the Email Agent',
    category: 'email',
    difficulty: 'beginner',
    estimatedTime: 15,
    prerequisites: [],
      ...
      sections: [
        {
          ...
          content: 'The Email Agent allows you to automate email communication across Gmail, Outlook, and custom IMAP/SMTP providers. This powerful tool enables you to streamline client communication, automate follow-ups, and process incoming emails programmatically.',
          tips: [
        ...
        },
        {
          ...
          content: 'Let\'s start by sending a simple email using the Email Agent. This is the foundation of email automation.',
          codeExamples: [
            {
...
await emailAgent.disconnect();`,
              language: 'typescript',
              explanation: 'This example demonstrates the basic flow: create agent, connect, send email, disconnect. The disconnect step is important to free up resources.'
            },
            {
  ...
  html: '<h1>Welcome!</h1><p>HTML version</p>'
              ...
              language: 'typescript',
              explanation: 'You can provide both plain text and HTML versions of your email for better compatibility.'
            }
          ]
        },
        {
          title: 'Reading Unread Emails',
          content: 'Fetch and process unread emails to automate responses or data extraction.',
          codeExamples: [
            {
...
for (const email of emails) {
  ...
  await emailAgent.markAsRead([email.id]);
              ...
              language: 'typescript',
              explanation: 'Fetch unread emails and process them systematically. Always mark emails as read after processing to avoid duplicates.'
            }
          ]
        }
      ]
...
await emailAgent.disconnect();`,
          explanation: 'Always connect first, perform the operation, then disconnect to free resources.'
        },
        validation: (userCode: string) => {
          ...
          if (hasConnect && hasSendEmail && hasWelcome && hasJoining) {
            return {
              passed: true,
              feedback: 'Excellent! You successfully sent a welcome email with proper connection handling.'
            };
          }
          ...
          if (!hasWelcome || !hasJoining) errors.push('Missing required email content');
          
          return {
            passed: false,
            feedback: 'Your solution is incomplete. Check the hints and try again.',
            errors
          };
        }
      }
      ...
      sections: [
        {
          ...
          content: 'The File Agent provides a unified interface for file operations across local and cloud storage systems. It handles path management, directory creation, and file operations with built-in error handling.',
          tips: [
        ...
        },
        {
          ...
          content: 'Organize your workspace by creating directory structures programmatically.',
          codeExamples: [
            {
...
await fileAgent.createDirectory({
  path: 'clients/acme-corp/projects/project-alpha'
              ...
              language: 'typescript',
              explanation: 'The createDirectory method creates all parent directories automatically (recursive: true by default).'
            }
          ]
        },
        {
...
console.log(content); // "Meeting notes from 2025-01-15"`,
              language: 'typescript',
              explanation: 'Files are written with automatic directory creation. Specify encoding for text files.'
            }
          ]
        }
      ]
...
await fileAgent.writeFile({
  ...
  encoding: 'utf-8'
          ...
          explanation: 'Creates the project structure and adds a markdown README with proper formatting.'
        },
        validation: (userCode: string) => {
          ...
          if (hasCreateDir && hasWriteFile && hasReadme) {
            return {
              passed: true,
              feedback: 'Perfect! You created the project structure with a README file.'
            };
          }
          
          return {
            passed: false,
            feedback: 'Make sure to create the directory and write the README file.',
      ...
      sections: [
        {
          ...
          content: 'The RSS Agent monitors industry news feeds and extracts client-relevant information automatically. It uses relevance scoring to prioritize the most important mentions.',
          tips: [
        ...
        },
        {
          ...
          content: 'Aggregate intelligence from multiple sources into actionable repositories.',
          codeExamples: [
            {
...
repository.items.forEach(item => {
  console.log(\`[\${item.relevanceScore.toFixed(2)}] \${item.title}\`);
              ...
              language: 'typescript',
              explanation: 'Builds a comprehensive repository from multiple RSS sources, with items ranked by relevance.'
            }
          ]
        }
      ]
  ...
  clientName: 'TechCorp'
          ...
          explanation: 'Monitors the feed for TechCorp mentions and builds a prioritized repository.'
        },
        validation: (userCode: string) => {
          if (userCode.includes('buildClientRepository') && 
              userCode.includes('TechCorp')) {
            return {
              passed: true,
              feedback: 'Excellent! You successfully monitored client mentions from RSS feeds.'
            };
          }
          return {
            passed: false,
      ...
      sections: [
        {
          ...
          content: 'Templates provide pre-built workflows for common automation tasks, saving time and ensuring consistency.',
          tips: [
...
// Workflow is now ready to execute`,
              language: 'typescript',
              explanation: 'Retrieves a template and applies client-specific variables to create an executable workflow.'
            }
          ]
        }
      ]
...
const workflow = templateRegistry.applyVariables(template.definition, {
  ...
  startDate: new Date().toISOString()
          ...
          explanation: 'Retrieves and applies the onboarding template with current date.'
        },
        validation: (userCode: string) => {
          if (userCode.includes('getTemplate') && 
...
function registerAdvancedWorkflowLesson(): void {
  const lesson: Lesson = {
    ...
    description: 'Learn to combine multiple agents into powerful automation workflows',
    category: 'workflow',
    difficulty: 'advanced',
    estimatedTime: 30,
    prerequisites: ['email-basics', 'file-basics', 'rss-basics', 'template-basics'],
    objectives: [
      ...
      sections: [
        {
          ...
          content: 'Create powerful automation by combining Email, File, and RSS agents in coordinated workflows.',
          tips: [
        ...
        },
        {
          ...
          content: 'Build an end-to-end workflow that gathers, stores, and reports intelligence.',
          codeExamples: [
            {
  ...
  try {
    ...
    throw error;
              ...
              language: 'typescript',
              explanation: 'A complete pipeline with error handling, logging, and proper data flow between agents.'
            }
          ]
        }
      ]
  ...
  throw error;
          ...
          explanation: 'Complete workflow with proper error handling and data flow.'
        },
        validation: (userCode: string) => {
          ...
          const hasErrorHandling = userCode.includes('try') && userCode.includes('catch');
          
          if (hasRss && hasFile && hasEmail && hasErrorHandling) {
            return {
              passed: true,
              feedback: 'Outstanding! You built a complete workflow with all components and error handling.'
            };
          }
          
          return {
            passed: false,
            feedback: 'Include all three agents (RSS, File, Email) and add error handling.',
