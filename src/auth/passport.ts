    ...
    try {
      ...
      if (oauthResult.rows.length > 0) {
        ...
        await db.query(
          ...
          [accessToken, refreshToken, profile.displayName, profile.photos?.[0]?.value, JSON.stringify(profile._json), 'google', profile.id]
        );
      } else {
        ...
        if (userResult.rows.length > 0) {
          ...
          const newUserResult = await db.query(
            `INSERT INTO users (email, full_name, avatar_url, license_key, is_verified)
             VALUES ($1, $2, $3, $4, true)
             RETURNING id`,
            [email.toLowerCase(), profile.displayName, profile.photos?.[0]?.value, licenseKey]
          );
        ...
        await db.query(
          ...
          [userId, 'google', profile.id, email, profile.displayName, profile.photos?.[0]?.value, accessToken, refreshToken, JSON.stringify(profile._json)]
        );
      }
...
if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
  ...
  }, async (accessToken: string, refreshToken: string, profile: OAuthProfile, done: (error: Error | null, user?: Express.User) => void) => {
    try {
      const email = profile.emails?.[0]?.value;
      if (!email) {
      ...
      if (oauthResult.rows.length > 0) {
        ...
        await db.query(
          ...
          [accessToken, profile.displayName, profile.photos?.[0]?.value, JSON.stringify(profile._json), 'github', profile.id]
        );
        ...
        if (userResult.rows.length > 0) {
          ...
          const newUserResult = await db.query(
            `INSERT INTO users (email, full_name, github_username, avatar_url, license_key, is_verified)
             ...
             RETURNING id`,
            [email.toLowerCase(), profile.displayName, profile.username, profile.photos?.[0]?.value, licenseKey]
          );
          userId = newUserResult.rows[0].id;
        }
        ...
        await db.query(
          ...
          [userId, 'github', profile.id, email, profile.displayName, profile.photos?.[0]?.value, accessToken, JSON.stringify(profile._json)]
        );
      }
