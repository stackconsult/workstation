...
export const RealTimePreview: React.FC<RealTimePreviewProps> = ({
  ...
  const executeWorkflow = useCallback(async () => {
    ...
    while (queue.length > 0 && isExecuting) {
      ...
      const stepDuration = Date.now() - (executionSteps[stepIndex]?.startTime || Date.now());
    ...
    const averageStepDuration = durations.reduce((a, b) => a + b, 0) / durations.length;
   ...
   */
  const simulateStepExecution = (node: any, currentVariables: Record<string, any>) => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: '5px 0 0 0', color: '#666' }}>{workflow.description}</p>
          </div>
          <button
            onClick={onClose}
            style={{
              ...
              <div style={{ marginTop: '30px' }}>
                ...
                <div style={{
                  ...
                  <div style={{ marginBottom: '10px' }}>
                    <strong>Total Duration:</strong> {performanceMetrics.totalDuration}ms
                  </div>
                  <div style={{ marginBottom: '10px' }}>
                    <strong>Steps:</strong> {performanceMetrics.stepCount}
                  </div>
                  <div style={{ marginBottom: '10px' }}>
                    <strong>Success Rate:</strong> {performanceMetrics.successRate.toFixed(1)}%
                  </div>
                  <div style={{ marginBottom: '10px' }}>
                    <strong>Avg Step Duration:</strong> {performanceMetrics.averageStepDuration.toFixed(0)}ms
                  </div>
                  <div style={{ marginBottom: '10px' }}>
                    <strong>Slowest Step:</strong> {performanceMetrics.slowestStep}
                  </div>
                  <div>
                    <strong>Fastest Step:</strong> {performanceMetrics.fastestStep}
                  </div>
                </div>
              </div>
            )}
