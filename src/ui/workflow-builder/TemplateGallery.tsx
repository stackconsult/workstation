...
export const TemplateGallery: React.FC<TemplateGalleryProps> = ({
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter(template => {
      ...
      const matchesComplexity = !complexityFilter || template.complexity === complexityFilter;
  ...
  return (
    <div className="template-gallery-modal" style={{
      position: 'fixed',
      top: 0,
      left: 0,
      right: 0,
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {TEMPLATE_CATEGORIES.map(category => (
              <button
                ...
                style={{
                  padding: '5px 15px',
                  background: selectedCategory === category.id ? '#2196F3' : '#f0f0f0',
