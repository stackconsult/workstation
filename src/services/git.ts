...
export class GitService {
  ...
  constructor(repoPath: string, githubToken?: string) {
    ...
    if (githubToken) {
      ...
      logger.warn('GitService initialized without GitHub token - PR operations will not be available');
    }
  }
   ...
   */
  async pushCurrentBranch(force: boolean = false): Promise<{ success: boolean; message: string }> {
    try {
      ...
      logger.info('Successfully pushed branch', { branch: currentBranch, force });
  ...
  async listPullRequests(state: 'open' | 'closed' | 'all' = 'open'): Promise<PullRequest[]> {
    if (!this.octokit) {
      throw new Error('GitHub token not configured - cannot fetch pull requests');
    }
  ...
  async createPullRequest(
    ...
    if (!this.octokit) {
      throw new Error('GitHub token not configured - cannot create pull request');
    }
    ...
    try {
      ...
      logger.info('Created pull request', { number: data.number, title, head, base });
  ...
  private async ensureRemote(): Promise<void> {
    try {
      ...
      } else if (origin.refs.push !== expectedUrl && origin.refs.fetch !== expectedUrl) {
   ...
   */
  async commitChanges(message: string, files?: string[]): Promise<{ success: boolean; message: string }> {
    try {
...
export function getGitService(repoPath?: string, githubToken?: string): GitService {
