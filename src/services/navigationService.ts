...
export class NavigationService {
  ...
  constructor(config: NavigationServiceConfig = {}) {
    const baseURL = config.baseURL || process.env.GO_BACKEND_URL || 'http://localhost:11434';
  ...
  async navigate(request: NavigationRequest): Promise<NavigationResponse> {
    try {
      const response = await this.client.post<NavigationResponse>('/api/browser/navigate', {
        url: request.url,
  ...
  async extractData(request: ExtractionRequest): Promise<Record<string, string>> {
    try {
      const response = await this.client.post<ExtractionResponse>('/api/browser/extract', {
        url: request.url,
  ...
  async click(request: ClickRequest): Promise<ClickResponse> {
    try {
      const response = await this.client.post<ClickResponse>('/api/browser/click', {
        url: request.url,
  ...
  async screenshot(request: ScreenshotRequest): Promise<string> {
    try {
      const response = await this.client.post<ScreenshotResponse>('/api/browser/screenshot', {
        url: request.url,
        selector: request.selector,
