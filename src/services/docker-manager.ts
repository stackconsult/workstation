...
class DockerManager {
  ...
  async restartContainer(nameOrId: string): Promise<void> {
    try {
      ...
      logger.error('Error restarting container', { container: nameOrId, error });
      throw error;
    }
  }
  ...
  async getContainerStats(nameOrId: string): Promise<ContainerStats> {
    try {
      ...
      logger.error('Error getting container stats', { container: nameOrId, error });
      throw error;
    }
  }
   ...
   * Get container logs
   */
  async getContainerLogs(nameOrId: string, tail: number = 100): Promise<string> {
    try {
      const container = this.docker.getContainer(nameOrId);
      ...
      logger.error('Error getting container logs', { container: nameOrId, error });
      throw error;
    }
  }
  ...
  async execInContainer(nameOrId: string, cmd: string[]): Promise<string> {
    try {
      ...
      logger.error('Error executing command in container', { container: nameOrId, cmd, error });
      throw error;
    }
  }
  ...
  async createContainer(options: {
    ...
    try {
      ...
      logger.info('Container created and started', { id: container.id, name: options.name });
      return container.id;
    } catch (error) {
