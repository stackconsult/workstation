...
router.get('/', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Get total count
    let countQuery = 'SELECT COUNT(*) FROM saved_workflows WHERE user_id = $1';
...
router.post('/', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const { name, description, category, actions, isTemplate = false } = req.body;
    ...
    const result = await db.query(
      `INSERT INTO saved_workflows 
       ...
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [userId, name, description || null, category || 'general', JSON.stringify(actions), isTemplate]
    );
