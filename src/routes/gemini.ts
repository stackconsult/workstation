...
const GenerateDisplaySchema = z.object({ workflowResult: z.record(z.string(), z.unknown()) });
const ChatSchema = z.object({ messages: z.array(z.object({ role: z.enum(['user', 'model']), content: z.string() })) });

// POST /api/gemini/natural-workflow
router.post('/natural-workflow', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!workflow) {
      return res.status(422).json({ success: false, error: 'Could not generate workflow' });
    }
...
router.post('/generate-display', async (req: Request, res: Response, next: NextFunction) => {
  try {
