 ...
 */
router.get('/status/:workspaceId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.delete('/disconnect/:workspaceId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { workspaceId } = req.params;
    ...
    logger.info('Slack disconnected', { workspaceId, userId: req.user?.userId });
