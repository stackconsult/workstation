 ...
 */
router.get('/entities', authenticateToken, async (req: Request, res: Response) => {
  try {
    const entityStore = getEntityStore();
    
    // Parse and validate type
    let entityType: 'file' | 'repository' | 'issue' | 'pr' | 'agent' | 'workflow' | 'user' | 'custom' | undefined;
    if (req.query.type) {
      const validTypes = ['file', 'repository', 'issue', 'pr', 'agent', 'workflow', 'user', 'custom'];
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to query entities'
    });
  }
});
...
router.get('/history', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const options = {
      workflow_id: req.query.workflow_id as string | undefined,
      status: req.query.status as 'success' | 'failure' | 'partial' | 'cancelled' | undefined,
 ...
 */
router.get('/history/:workflowId/stats', authenticateToken, async (req: Request, res: Response) => {
  try {
    const workflowHistory = getWorkflowHistory();
    const stats = await workflowHistory.getWorkflowStatistics(req.params.workflowId);
    ...
    logger.error('Failed to get workflow statistics', { error, workflowId: req.params.workflowId });
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to get statistics'
    });
 ...
 */
router.get('/patterns/:workflowId', authenticateToken, async (req: Request, res: Response) => {
  try {
    const workflowHistory = getWorkflowHistory();
    const patterns = await workflowHistory.getWorkflowPatterns(req.params.workflowId);
    ...
    logger.error('Failed to get workflow patterns', { error, workflowId: req.params.workflowId });
    res.status(500).json({
      success: false,
 ...
 */
router.get('/suggestions/:workflowId', authenticateToken, async (req: Request, res: Response) => {
  try {
    const learningModel = getLearningModel();
    const suggestions = await learningModel.getWorkflowSuggestions(req.params.workflowId);
    ...
    logger.error('Failed to get suggestions', { error, workflowId: req.params.workflowId });
    res.status(500).json({
      success: false,
...
router.post('/suggestions/:id/apply', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to apply suggestion', { error, suggestionId: req.params.id });
    res.status(500).json({
      success: false,
