 ...
 */
router.post('/:id/health', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    await agentOrchestrator.updateAgentHealth(id, healthStatus, metadata || {});
...
router.post('/tasks', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    logger.info(`Task ${taskId} created for agent ${agentId} by user ${userId}`);
  } catch (error) {
 ...
 */
router.get('/:id/tasks', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { limit = 50 } = req.query;

    const tasks = await agentOrchestrator.getAgentTasks(id, parseInt(limit as string));
 ...
 */
router.get('/:id/statistics', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const stats = await agentOrchestrator.getAgentStatistics(id);
 ...
 */
router.get('/system/overview', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const overview = {
      ...
      unhealthyAgents: agents.filter(a => a.healthStatus !== 'healthy').length,
      pendingTasks
    };
