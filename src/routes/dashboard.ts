    ...
    res.json({
      ...
      data: {
        statistics: {
          ...
          avgExecutionTime: stats.avg_execution_time ? parseFloat(stats.avg_execution_time) : 0
        },
        agentStatistics: {
...
router.get('/analytics', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    logger.info(`Analytics data retrieved for user ${userId}, period ${start} to ${end}`);
  } catch (error) {
