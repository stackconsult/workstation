...
The following workflows have been temporarily disabled. See [DISABLED_WORKFLOWS_RESOLUTION.md](./DISABLED_WORKFLOWS_RESOLUTION.md) for comprehensive analysis and resolution plan.
...
1. **agent-doc-generator.yml.disabled** - AI Agent Builder Documentation Generator
   ...
   - Solution: Needs rewriting to use external shell scripts (see resolution doc)
...
4. **auto-fix-dependencies.yml.disabled** - DELETED (Nov 14, 2025)
   - Status: Removed permanently as it contradicts CI/CD best practices
   - Reason: Automatically replaced `npm ci` with `npm install`, which is incorrect
   - Why harmful: `npm ci` is the correct command for CI/CD as it provides consistent, reproducible builds
   - Action taken: File deleted, not just disabled
...
For detailed analysis, resolution options, and implementation steps, see:
**[DISABLED_WORKFLOWS_RESOLUTION.md](./DISABLED_WORKFLOWS_RESOLUTION.md)**
