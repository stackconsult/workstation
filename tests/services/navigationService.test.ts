...
import { NavigationService, NavigationRequest, ExtractionRequest } from '../../src/services/navigationService';
...
describe('NavigationService', () => {
  ...
  describe('navigate', () => {
    it('should successfully navigate to URL', async () => {
      ...
      expect(mockAxiosInstance.post).toHaveBeenCalledWith('/api/browser/navigate', {
        url: 'https://example.com',
        selectors: ['h1', 'body'],
        timeout: 30,
      });
    ...
    it('should throw error when navigation fails', async () => {
      ...
      await expect(service.navigate(request)).rejects.toThrow('Navigation failed: Timeout');
    });
  });
