...
describe('Workstation Integration Tests', () => {
  describe('File Structure', () => {
    ...
    test('should have agent-server package.json', () => {
      const packagePath = path.join(process.cwd(), 'agent-server', 'nodejs', 'package.json');
      expect(fs.existsSync(packagePath)).toBe(true);
    });

    test('should have agent-server start.js', () => {
      const startPath = path.join(process.cwd(), 'agent-server', 'nodejs', 'start.js');
  ...
  describe('Docker Configuration', () => {
    ...
    test('should have integrated Docker Compose file', () => {
      const composePath = path.join(process.cwd(), 'docker-compose.integrated.yml');
      expect(fs.existsSync(composePath)).toBe(true);
    });

    test('should have start-services script', () => {
      const scriptPath = path.join(process.cwd(), 'docker', 'start-services.sh');
