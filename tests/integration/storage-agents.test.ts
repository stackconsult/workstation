    ...
    it('should export database table to S3 as CSV', async () => {
      const mockWorkflow = jest.fn().mockResolvedValue({
        ...
        steps: [
          ...
          { agent: 's3', action: 'uploadFile', key: 'backups/users-2024-01.csv' }
        ],
    ...
    it('should handle large file import with batching', async () => {
      const mockWorkflow = jest.fn().mockResolvedValue({
        ...
        steps: [
          ...
          { agent: 'database', action: 'insert', batches: 50, rowsInserted: 50000 }
        ],
        batchSize: 1000,
        totalBatches: 50,
        duration: 30000
  ...
  describe('Database synchronization across environments', () => {
    it('should sync data between production and staging', async () => {
      const mockWorkflow = jest.fn().mockResolvedValue({
        success: true,
        steps: [
          { agent: 'database', action: 'query', source: 'production', rows: 200 },
          { agent: 'database', action: 'transaction', target: 'staging', operations: 200 }
        ],
        rowsSynced: 200,
        conflicts: 0
      });
    ...
    it('should migrate from SQLite to PostgreSQL', async () => {
      const mockWorkflow = jest.fn().mockResolvedValue({
        ...
        steps: [
          ...
          { agent: 'database', action: 'insert', target: 'postgresql', rows: 5000 }
        ],
        rowsMigrated: 5000,
        tablesProcessed: 5
      });
    ...
    it('should process S3 files in database transaction', async () => {
      const mockWorkflow = jest.fn().mockResolvedValue({
        ...
        steps: [
          ...
          { agent: 'database', action: 'transaction', operations: 10, committed: true }
        ],
        filesProcessed: 5,
    ...
    it('should use indexes for large queries', async () => {
      const mockWorkflow = jest.fn().mockResolvedValue({
        ...
        steps: [
          ...
          { agent: 'database', action: 'query', usedIndexes: true, duration: 100 }
        ],
        queryOptimized: true,
        executionTime: 100
      });
