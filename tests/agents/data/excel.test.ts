...
import { ExcelAgent, excelAgent } from '../../../src/automation/agents/data/excel';
...
describe('Excel Agent', () => {
  ...
  describe('listSheets', () => {
    it('should list all sheets in workbook', async () => {
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['Data1']]), 'First');
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['Data2']]), 'Second');
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['Data3']]), 'Third');
      const buffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
    ...
    it('should handle workbook with single sheet', async () => {
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['Data']]), 'Only');
      const buffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
  ...
  describe('formatCells', () => {
    it('should format cells in workbook', async () => {
      ...
      const result = await agent.formatCells({
        input: buffer,
        formats: [
          { cell: 'A1', format: { bold: true, color: 'red' } }
        ]
      });
