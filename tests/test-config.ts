 ...
 */
export async function isServiceAvailable(serviceName: keyof typeof TEST_CONFIG.services): Promise<boolean> {
  ...
  if (serviceName === 'redis') {
    try {
      const redisService = service as { enabled: boolean; required: boolean; host: string; port: number };
 ...
 */
export function skipIfServiceUnavailable(serviceName: keyof typeof TEST_CONFIG.services) {
  return async () => {
