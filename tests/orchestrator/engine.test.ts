...
describe('Orchestration Engine', () => {
  ...
  describe('executeWorkflow', () => {
    ...
    it('should reject inactive workflow', async () => {
      const mockExecute = jest.fn().mockRejectedValue(
        new Error('Workflow is not active: disabled')
      );
    ...
    it('should handle task failures', async () => {
      const mockRun = jest.fn().mockResolvedValue({
        ...
        tasks: [
          ...
          { name: 'task2', status: 'failed', error_message: 'Task execution error' }
        ],
  ...
  describe('agent integration', () => {
    ...
    it('should handle action not supported', async () => {
      const mockExecute = jest.fn().mockRejectedValue(
        new Error('Action not supported: invalidAction')
      );
  ...
  describe('error handling', () => {
    ...
    it('should handle JSON parsing errors', async () => {
      const mockExecute = jest.fn().mockRejectedValue(
        new Error('Invalid JSON in workflow definition')
      );
  ...
  describe('workflow definition validation', () => {
    it('should validate workflow structure', async () => {
      ...
      const result = await mockValidate({
        tasks: [
          { name: 'task1', action: 'test', order: 0 }
        ]
      });
