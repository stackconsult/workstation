...
describe('Parallel Execution Engine', () => {
  ...
  describe('DAG construction', () => {
    ...
    it('should build DAG with simple dependencies', () => {
      const tasks: WorkflowTask[] = [
        { name: 'task1', action: 'test', params: {}, order: 0, depends_on: [] },
        { name: 'task2', action: 'test', params: {}, order: 1, depends_on: ['task1'] },
        { name: 'task3', action: 'test', params: {}, order: 2, depends_on: ['task2'] }
      ];
    ...
    it('should build DAG with diamond dependency pattern', () => {
      const tasks: WorkflowTask[] = [
        ...
        { name: 'left', action: 'test', params: {}, order: 1, depends_on: ['start'] },
        { name: 'right', action: 'test', params: {}, order: 2, depends_on: ['start'] },
        { name: 'end', action: 'test', params: {}, order: 3, depends_on: ['left', 'right'] }
      ];
    ...
    it('should populate dependents lists', () => {
      const tasks: WorkflowTask[] = [
        { name: 'task1', action: 'test', params: {}, order: 0, depends_on: [] },
        { name: 'task2', action: 'test', params: {}, order: 1, depends_on: ['task1'] },
        { name: 'task3', action: 'test', params: {}, order: 2, depends_on: ['task1'] }
      ];
    ...
    it('should detect circular dependencies', () => {
      const tasks: WorkflowTask[] = [
        { name: 'task1', action: 'test', params: {}, order: 0, depends_on: ['task3'] },
        { name: 'task2', action: 'test', params: {}, order: 1, depends_on: ['task1'] },
        { name: 'task3', action: 'test', params: {}, order: 2, depends_on: ['task2'] }
      ];
    ...
    it('should detect self-referencing task', () => {
      const tasks: WorkflowTask[] = [
        { name: 'task1', action: 'test', params: {}, order: 0, depends_on: ['task1'] }
      ];
  ...
  describe('execution levels', () => {
    it('should assign correct levels for linear chain', () => {
      const tasks: WorkflowTask[] = [
        ...
        { name: 't2', action: 'test', params: {}, order: 1, depends_on: ['t1'] },
        { name: 't3', action: 'test', params: {}, order: 2, depends_on: ['t2'] },
        { name: 't4', action: 'test', params: {}, order: 3, depends_on: ['t3'] }
      ];
    ...
    it('should assign correct levels for parallel branches', () => {
      const tasks: WorkflowTask[] = [
        { name: 'start', action: 'test', params: {}, order: 0, depends_on: [] },
        { name: 'branch1', action: 'test', params: {}, order: 1, depends_on: ['start'] },
        { name: 'branch2', action: 'test', params: {}, order: 2, depends_on: ['start'] },
        { name: 'branch3', action: 'test', params: {}, order: 3, depends_on: ['start'] }
      ];
    ...
    it('should handle complex dependency graph', () => {
      const tasks: WorkflowTask[] = [
        ...
        { name: 'd', action: 'test', params: {}, order: 3, depends_on: ['b', 'c'] },
        { name: 'e', action: 'test', params: {}, order: 4, depends_on: ['c'] },
        { name: 'f', action: 'test', params: {}, order: 5, depends_on: ['d', 'e'] }
      ];
  ...
  describe('parallel execution (mocked)', () => {
    ...
    it('should handle task failure and stop dependents', async () => {
      const mockExecute = jest.fn().mockResolvedValue({
        ...
        failed: [
          { id: 't2', status: 'failed', error: 'Task execution error' }
        ],
        skipped: [
          { id: 't3', status: 'skipped', reason: 'Dependency failed' }
        ],
        totalDuration: 150
      });
    ...
    it('should pass data between dependent tasks', async () => {
      const mockExecute = jest.fn().mockResolvedValue({
        completed: [
          ...
          { id: 't2', status: 'completed', result: { profileData: { userId: 123 } } }
        ],
        failed: [],
        totalDuration: 200
      });
      ...
      expect(result.completed[1].result).toEqual({ profileData: { userId: 123 } });
    });
  });
  ...
  describe('error handling', () => {
    it('should handle missing dependency gracefully', () => {
      const tasks: WorkflowTask[] = [
        { name: 't1', action: 'test', params: {}, order: 0, depends_on: ['nonexistent'] }
      ];
  ...
  describe('edge cases', () => {
    it('should handle tasks with identical names', () => {
      const tasks: WorkflowTask[] = [
        { name: 'duplicate', action: 'test1', params: {}, order: 0, depends_on: [] },
        { name: 'duplicate', action: 'test2', params: {}, order: 1, depends_on: [] }
      ];
